...
@app.command("init")
def init_cmd(
    workspace: Path = typer.Option(Path("."), "--workspace", help="Project workspace root."),
    format: str = typer.Option("text", "--format", help="Output format: text|json."),
) -> None:
    init.run(workspace=workspace, fmt=format)
...
def ingest_cmd(
    ...
    ingest.run(workspace=workspace, src=src, ext=ext, recursive=recursive, fmt=format)
...
def query_cmd(
    ...
    query.run(workspace=workspace, question=question, mode=mode, top_k=top_k, fmt=format)
...
def docs_cmd(
    ...
    docs.run(workspace=workspace, fmt=format)
